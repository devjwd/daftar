...
export const jsonResponse = (body: Record<string, unknown>, status = 200, req?: Request) =>
...
export const verifyAdminRequest = async (
  ...
  if (!claimedAddress || !publicKey || !signature || !messageB64 || !fullMessageB64) {
    return { ok: false, response: jsonResponse({ error: 'Missing admin proof headers' }, 401, req) }
  }
  ...
  try {
    ...
    return { ok: false, response: jsonResponse({ error: 'Invalid admin proof payload' }, 401, req) }
  }
  ...
  if (!Number.isFinite(issuedAt) || Math.abs(Date.now() - issuedAt) > ADMIN_SIGNATURE_TTL_MS) {
    return { ok: false, response: jsonResponse({ error: 'Admin proof has expired' }, 401, req) }
  }

  if (!nonce || action !== expectedAction) {
    return { ok: false, response: jsonResponse({ error: 'Admin proof action mismatch' }, 401, req) }
  }

  const expectedHash = await sha256Hex(stableStringify(payload))
  if (bodyHash !== expectedHash) {
    return { ok: false, response: jsonResponse({ error: 'Admin proof body hash mismatch' }, 401, req) }
  }

  if (!fullMessage.includes(`message: ${message}`) || !fullMessage.includes(`nonce: ${nonce}`)) {
    return { ok: false, response: jsonResponse({ error: 'Signed message content mismatch' }, 401, req) }
  }
  ...
  try {
    ...
    return { ok: false, response: jsonResponse({ error: 'Invalid admin signature' }, 401, req) }
  }

  if (!verified || claimedAddress !== derivedAddress || derivedAddress !== allowedAddress) {
    return { ok: false, response: jsonResponse({ error: 'Unauthorized admin signer' }, 401, req) }
  }
