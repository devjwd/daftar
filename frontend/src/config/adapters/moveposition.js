...
function logAllNumericValues(data) {
  ...
  if (values.length > 0) {
    console.log("ðŸ“‹ ALL NUMERIC VALUES IN DATA:\n" + values.slice(0, 30).join("\n"));
...
export const movePositionAdapter = [
  {
    ...
    parse: (data) => {
      try {
        ...
        console.log("ðŸ” MovePosition Supply - Raw data keys:", Object.keys(data));
        console.log("ðŸ” Full data structure:", JSON.stringify(data).substring(0, 500));
        ...
        if (extractedCoins.length > 0) {
          console.log("âœ… Found coins via extraction:", extractedCoins.map(c => `${c.symbol}(${c.amount})`).join(", "));
        ...
        for (const field of supplyFields) {
          ...
          console.log(`ðŸ” Checking field: ${field}`, fieldData);
          ...
          for (const key of keyVariants) {
            ...
            if (amount >= 1) {
              if (!positions.find(p => p.symbol === tokenName)) {
                ...
                console.log(`âœ… Found ${tokenName} supply: ${amount} = ${formatted}`);
                positions.push({
                  token: tokenName,
                  symbol: tokenName,
        ...
        console.log("ðŸ“Š Final supply positions found:", positions.map(p => `${p.symbol}:${p.formattedAmount}`).join(", "));
...

  {
    ...
    parse: (data) => {
      try {
        ...
        console.log("ðŸ” MovePosition Borrow - Raw data keys:", Object.keys(data));
        console.log("ðŸ” Full data structure:", JSON.stringify(data).substring(0, 500));
        ...
        for (const [tokenName, tokenInfo] of Object.entries(MOVEPOSITION_TOKENS)) {
          ...
          for (const key of keyVariants) {
            ...
            if (amount >= 1) {
              if (!positions.find(p => p.symbol === tokenName)) {
                const formatted = amount / Math.pow(10, tokenInfo.decimals);
                console.log(`âœ… Found ${tokenName} borrow: ${amount} = ${formatted}`);
                positions.push({
                  token: tokenName,
                  symbol: tokenName,
                  coinType: tokenInfo.coinType,
        ...
        console.log("ðŸ“Š Final borrow positions found:", positions.map(p => `${p.symbol}:${p.formattedAmount}`).join(", "));
        return positions.length > 0 ? positions : "0";
      } catch (e) {
