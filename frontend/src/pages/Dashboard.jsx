...
const Dashboard = () => {
  ...
  const visibleLiquidityPositions = useMemo(() => {
    if (!Array.isArray(liquidityPositions) || liquidityPositions.length === 0) return [];

    return liquidityPositions.filter((position) =>
      shouldDisplayPosition(getLiquidityPositionUsdValue(position, priceMap), hidePositionThreshold)
    );
  }, [hidePositionThreshold, liquidityPositions, priceMap]);
  ...
  const liquidityTotalValue = useMemo(() => {
    if (!visibleLiquidityPositions || visibleLiquidityPositions.length === 0) return 0;

    return visibleLiquidityPositions.reduce((total, position) => {
      const value = getLiquidityPositionUsdValue(position, priceMap) ?? 0;
      return total + value;
    }, 0);
  }, [priceMap, visibleLiquidityPositions]);
        ...
        try {
          ...
          for (const nft of nftHoldings) {
            ...
            if (isYuzuPosition) {
              ...
              devLog("Yuzu position info:", { positionId, poolPair, poolAddress: poolAddress.substring(0, 20) + '...' });
                ...
                try {
                  ...
                  if (response.ok) {
                    ...
                    if (Array.isArray(result) && result.length >= 2) {
                      ...
                      devLog('Yuzu token decimals:', { token0: tokens[0], decimals0, token1: tokens[1], decimals1 });
    ...
    <>
            ...
            <section className="grid-section">
              ...
              <div className="grid-container lp-grid">
                ...
                {!indexerLoading && visibleLiquidityPositions.length === 0 && viewingAddress && (
