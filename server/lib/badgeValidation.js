...
const ensureString = (value, fallback = '') => String(value ?? fallback).trim();

const ensureBoolean = (value, fallback = true) =>
  typeof value === 'boolean' ? value : fallback;

const ensureIntegerInRange = (value, min, max, fallback) => {
...
const ensureNumberInRange = (value, min, max, fallback) => {
  const parsed = Number(value);
  if (!Number.isFinite(parsed)) return fallback;
...
export const normalizeRuleType = (value) => {
...
const validateStringLength = (value, maxLength, label) => {
  if (value.length > maxLength) {
    throw new Error(`${label} must be ${maxLength} characters or less`);
  }
};

const validateJsonSize = (value, label) => {
...
const sanitizeRuleCriteria = (value, allowComposite = true) => {
...
export const getSafeErrorMessage = (fallback) => {
  return fallback;
};
