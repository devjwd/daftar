...
const ensureString = (value: any, fallback: string = ''): string => String(value ?? fallback).trim();

const ensureBoolean = (value: any, fallback: boolean = true): boolean =>
  typeof value === 'boolean' ? value : fallback;

const ensureIntegerInRange = (value: any, min: number, max: number, fallback: number): number => {
...
const ensureNumberInRange = (value: any, min: number, max: number, fallback: number): number => {
  const parsed = Number(value);
  if (!Number.isFinite(parsed)) return fallback;
  return Math.min(max, Math.max(min, parsed));
};

const cloneJsonValue = (value: any): any => JSON.parse(JSON.stringify(value));
...
export const normalizeRuleType = (value: any): number | null => {
...
const validateStringLength = (value: string, maxLength: number, label: string): void => {
  if (value.length > maxLength) {
    throw new Error(`${label} must be ${maxLength} characters or less`);
  }
};

const validateJsonSize = (value: any, label: string): void => {
...
const sanitizeRuleCriteria = (value: any, allowComposite: boolean = true): any[] => {
  ...
  try {
    ...
    if (ruleType === BADGE_RULES.DAFTAR_SWAP_COUNT) {
      return {
        ok: true,
        ruleParams: {
          min: ensureIntegerInRange(params.min ?? params.count ?? 1, 1, 100000, 1),
        },
      };
    }
  ...
  try {
    ...
    const onChainBadgeId =
      onChainBadgeIdRaw == null || onChainBadgeIdRaw === ''
        ? null
        : ensureIntegerInRange(onChainBadgeIdRaw, 0, 1000000000, 0);

    const xpValue = ensureIntegerInRange(badge.xp_value ?? badge.xp, 0, 1000000, 0);
    const mintFee = ensureNumberInRange(badge.mint_fee ?? badge.mintFee, 0, 1000000000, 0);
...
export const getSafeErrorMessage = (fallback: string): string => {
  return fallback;
};
