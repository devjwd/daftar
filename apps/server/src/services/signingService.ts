...
import { MintSignatureResponse } from '@daftar/types';

/**
 * Generates a signed mint authorization payload for the badges Move contract.
 * Message structure: domain || module_addr || user_addr || badge_id || valid_until || signer_epoch || nonce
 */
export async function signMintAuthorization(
  ...
  if (!moduleAddress) throw new Error('Badge module address is required for signing');
  ...
  const sNonce = new Serializer();
  sNonce.serializeU64(BigInt(nonce));
  const nonceBytes = sNonce.toUint8Array();
  
  // Total size: domain + 2 addresses (32+32) + 4 u64s (8*4)
