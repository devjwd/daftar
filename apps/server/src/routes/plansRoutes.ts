...
import { queueSync, drainSyncQueue } from '../services/analyticsSyncQueue.ts';
...
router.post('/verify-payment', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 9. Queue analytics sync with high priority — triggers immediate parallel drain
    ...
    try {
      ...
      // Drain up to 5 jobs in parallel in the background.
      // This ensures the new Pro user isn't stuck behind a long serial queue.
      drainSyncQueue(supabase, 5).catch(err => {
        console.error('[Plans] Background queue drain failed:', err.message);
      });
    } catch (syncErr: any) {
