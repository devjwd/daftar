...
export const normalizeMosaicSwapSettings = (rawSettings = {}) => {
  ...
  const rawChargeFeeBy = String(rawSettings.chargeFeeBy || "").trim().toLowerCase();
  const chargeFeeBy = rawChargeFeeBy === "token_out"
    ? "token_out"
    : rawChargeFeeBy === "token_in"
      ? "token_in"
      : (rawSettings.isFeeIn === false ? "token_out" : "token_in");
...
import { normalizeAddress as baseNormalizeAddress } from '../utils/address';
...
export const fetchMosaicQuote = async ({
  ...
  if (feeBps > 0) {
    ...
    params.set("chargeFeeBy", String(settings.chargeFeeBy || (settings.isFeeIn === false ? "token_out" : "token_in")));
