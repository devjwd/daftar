...
import { clearEligibilityCache } from './badges/engineService';
...
export const decodeBytes = (value: any) => {
...
export const buildMetadataJson = ({ name, description, imageUri, externalUrl, attributes }: any) => {
...
export const buildMetadataDataUri = (json: string) => {
  return `data:application/json;base64,${encodeBase64(json)}`;
};

export const computeSha256Hex = async (input: string) => {
  try {
 ...
 * waitForTxAndGetId
 */
export const waitForTxAndGetId = async (client: any, txHash: string, options: any = {}) => {
  ...
  try {
    ...
    const event = tx.events?.find((e: any) => e.type === eventType);
    ...
    if (registryInfo && registryInfo.nextId > 0) {
      ...
      if (badgeDetails && options.creator && normalizeAddress((badgeDetails as any).creator) === normalizeAddress(options.creator)) {
...
export const setFeeTreasury = async ({
  ...
  sender,
  newTreasury,
}: { signAndSubmitTransaction: any, sender: string, newTreasury: string }) => {
